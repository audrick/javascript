...
while(response != 'q')   // using 'while' instead of 'if' to loop the question until 'q' is pressed.  
{
	var count = prompt('How many would you like?');
	count = parseInt(count); //need to turn string into a number

	if(response == 'a')
	{
		for(var i =0; i < count; i++)  //incrementing i here
		{
...
while(response != 'q')
{
	...
	response = prompt('Do you want to remove (c)color, (p)erson, (a)nimal or (q)uit?');
}
